/*! Phlox Core Plugin - v2.3.0 (2018-11-19)
...
;(function ( $, window, document, undefined ) {
    ...
    $.extend(Plugin.prototype, {
        init: function () {
            ...
            // Steps Manager Event
            $(document).on( "click", ".aux-next-step", this._stepManager.bind(this) );

            // Install & Uninstall Demo Events
            $(document).on( "click", ".aux-install-demo", this._demoManager.bind(this) );
            $(document).on( "click", ".aux-uninstall-demo", this._uninstallDemo.bind(this) );

            // Template Manager Event
            $(document).on( "click", ".aux-copy-template", this._tempManager.bind(this) );

            // Install Plugins Event
            $(document).on( "click", ".install-plugins", this._pluginManager.bind(this) );
        ...
        _openModal: function(){
            ...
            $( self.settings.modalClass ).featherlight({
                targetAttr    : "href",
                closeOnEsc    : false,
                closeOnClick  : false,
                contentFilters: ["ajax"],
        ...
        _changeGroup: function( e ){
            ...
            this._globalAJAX( function( response ){
                if( response !== null && response.success ){
                    this.$isotopeTemplate.AuxIsotope( "changeGroup", groupName );
                } else {
        ...
        _stepManager: function( e ){
            ...
            $modalSectionElement = this.$modalElement.find('.aux-steps-col');
            this._ajaxData = {
            ...
            this._globalAJAX( function( response ){
                if( response !== null && response.success ){
                    $modalSectionElement.removeClass("aux-step-in-progress").html( response.data.markup );
                    // Run hidden action
                    if( this._ajaxData.next_action ) {
                        this._tempManager( { currentTarget: this._modalButton } );
                    }
                } else {
                    console.log( response );
                }
        ...
        _tempManager: function( e ){
            ...
            // Set variables
            var $buttonElement   = $( e.currentTarget ),
            $modalSectionElement = this.$modalElement != null ? this.$modalElement.find('.aux-steps-col') : null;
            this._ajaxData = {
            ...
            this._globalAJAX( function( response ){
                if( response !== null && response.success ){
                    if( response.data.status === "copy" ){
                        ...
                        this._updateElementorLocalStorage( this._ajaxData.type, response.data.result.content );
                        // Then do these actions
                        $buttonElement.addClass("aux-button-success");
                        setTimeout(function () {
                            $buttonElement.removeClass("aux-button-success aux-button-loading")
                        }, 1000);
                    } else {
                        // Change button type to copy
                        $buttonElement.data( "status-type", "copy" )
                        .prop( "data-status-type", "copy" )
                        .addClass("aux-copy-template aux-orange")
                        .removeClass("aux-import-template aux-has-next-action aux-green2")
                        .removeAttr("href");
                        // Change button text to copy
                        $buttonElement.find( "span" ).text( response.data.label );
                        // Display the more button
                        $buttonElement.next( ".aux-more-button" ).removeClass( "hide" );
        ...
        _demoManager: function( e ){
            ...
            // Set variable
            var $buttonElement = $( e.currentTarget ),
            $buttonParentEl    = $buttonElement.closest( ".aux-setup-demo-actions" ),
            ...
            $demoProgress      = this.$modalElement.find( ".aux-install-demos-waiting" );
            this._ajaxData     = {
                action : "auxin_demo_data",
                ID     : $buttonElement.data("import-id"),
                verify : $buttonElement.data("nonce"),
                options: $optionsElement.find(".aux-import-parts").serializeArray()
            };
            this.$progressBarLabelEl = $progressBarEl.find( ".aux-progress-label" );
                ...
                if( response !== null && response.success ){
                    if ( response.data.next !== "final" ) {
                        this._demoImport({
                            ...
                            index  : response.data.hasOwnProperty("index") ? response.data.index : ""
                        });
                    } else {
                        this.$progressBarLabelEl.text( response.data.message );
                        setTimeout( function() {
                            this._controlActions( "on" );
                            // Next Step Trigger
                            this._stepManager( { currentTarget: data.target } );
                        }.bind(this), 1000 );
                    }
                } else {
                    console.log( response );
        ...
        _uninstallDemo: function( e ){
            ...
            this._globalAJAX( function( response ){
                $buttonParentEl.find(".aux-return-back").removeClass("hide");
                $buttonParentEl.find(".aux-progress").addClass("hide");

                if( response !== null && response.success ){
                    this.$modalElement.find('.aux-steps-col').html( response.data.markup );
                    $(this._modalButton).removeClass('aux-uninstall aux-orange')
                    .addClass('aux-green2').text( response.data.button )
        ...
        _pluginManager: function( e ){
            ...
            // Set variable
            var $buttonElement       = $( e.currentTarget );
            this.$buttonParentEl     = $buttonElement.closest( ".aux-has-required-plugins" );
            this.$pluginsListEl      = this.$buttonParentEl.find(".aux-wizard-plugins");
            this._selectedPluginsNum = this.$buttonParentEl.find('.aux-plugin input[name="plugin[]"]:checked').length;
            ...
            $buttonElement.text(aux_setup_params.btnworks_text).addClass("disabled");
        ...
        _processPlugins: function(){
            ...
            $pluginsList.each(function(){
                if( self._currentItem == null || doNext ){
                    if( $(this).find('input[name="plugin[]"]').is(":checked") ) {
                        ...
                        self.$currentNode.find(".spinner").css("visibility","visible");
                        self._installPlugin();
                        doNext = false;
                    }
            ...
            if( this._itemsCompleted >= this._selectedPluginsNum ){
                ...
                this.$buttonParentEl.find(".aux-wizard-plugins").removeClass("installing");
                ...
                this.$buttonParentEl.find(".skip-next").text(aux_setup_params.nextstep_text);
                // Continue loading process
                if( this.$buttonParentEl.find(".aux-plugin").not(".aux-success").length == 0 && this.$buttonParentEl.hasClass("aux-modal-item") ){
        ...
        _installPlugin: function(){
            if( this._currentItem ){
                var plugins  = this.$buttonParentEl.find('.aux-wizard-plugins input[name="plugin[]"]:checked').map(function(){
                    return $(this).val();
                }).get();
                this._ajaxData = {
        ...
        _pluginActions: function( response ){
            ...
            if( typeof response === "object" && response.success ){
                ...
                this.$currentNode.find(".column-status span").text( response.data.message );
                ...
                if( typeof response.data.url !== "undefined" ){
                    ...
                    if( this.currentItemHash ==  response.data.hash ) {
                        this.$currentNode.data("done_item", 0).find(".column-status span").text("failed");
                        this._processPlugins();
                    } else {
                        ...
                        this._globalAJAX( function( html ){
                    ...
                    this.$currentNode.addClass("aux-success").find(".aux-check-column").remove();
                    this.$currentNode.find(".check-column").append("<i class='aux-success-icon auxicon-check-mark-circle-outline'></i>");
                ...
                if( this._attemptsBuffer > 1 ) {
                    ...
                    this.$currentNode.addClass("aux-error").find(".column-status span").text("Ajax Error!");
                    this._processPlugins();
                } else {
        ...
        _manipulations: function(){
            ...
            $('.aux-wizard-plugins input[name="plugin[]"]').each(function() {
                if($(this).is(':checked')) {
                    $(this).closest('.aux-table-row').addClass('is-checked');
                } else {
                     $(this).closest('.aux-table-row').removeClass('is-checked');
                }
                $(this).click(function() {
                    if($(this).is(':checked')) {
                        $(this).closest('.aux-table-row').addClass('is-checked');
            ...
            $(document).on('click', '.aux-more-button', function(e) {
                e.preventDefault();
                $(this).next('.aux-more-items').toggleClass('aux-display');
            });
